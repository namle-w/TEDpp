...
def get_dir_core(args, include_model_name=False, include_poison_seed=False):
    ...
    elif args.poison_type == 'adaptive_patch' or args.poison_type == 'TaCT' or args.poison_type == 'WaNet' or args.poison_type == 'Mirror' or args.poison_type == 'radialblend':
        cover_rate = '%.3f' % args.cover_rate
...
def get_poison_set_dir(args):
    ...
    if args.poison_type in ['trojannn', 'BadEncoder', 'SRA']:
        ...
        radialblend_alpha = '%.3f' % args.alpha
        cover_rate = '%.3f' % args.cover_rate
        poison_set_dir = 'poisoned_train_set/%s/%s_%s_alpha=%s_cover=%s' % (
        args.dataset, args.poison_type, ratio, radialblend_alpha, cover_rate)
    elif args.poison_type == 'adaptive_patch' or args.poison_type == 'TaCT' or args.poison_type == 'WaNet' or args.poison_type == 'Mirror' or args.poison_type == 'radialblend':
        cover_rate = '%.3f' % args.cover_rate
...
def get_poison_transform(poison_type, dataset_name, target_class, source_class=1, cover_classes=[5, 7],
    ...
    if poison_type in ['basic', 'badnet', 'blend', 'clean_label', 'refool',
                       ...
                       'badnet_all_to_all', 'trojan', 'SRA', 'bpp', 'Mirror', 'radialblend']:
        ...
        if trigger_name != 'none':  # none for SIG
            ...
            if dataset_name == 'mnist':
                trigger = Image.open(trigger_path).convert("L")
            else:
            ...
            if os.path.exists(trigger_mask_path):
                ...
                if dataset_name == 'mnist':
                    ...
                    trigger_mask = (trigger_map > 0).float()
                else:
        ...
        if poison_type == 'basic':
            ...
            poison_transform = WaNet.poison_transform(img_size=img_size, denormalizer=denormalizer,
                                                      ...
                                                      target_class=target_class)
        elif poison_type == 'Mirror':
            kappa = 0.1
            cx = 0.5
            cy = 0.5

            from poison_tool_box import Mirror
            poison_transform = Mirror.poison_transform(img_size=img_size, denormalizer=denormalizer, kappa=kappa, cx=cx, cy=cy, normalizer=normalizer,
                                                      target_class=target_class)

        elif args.poison_type == 'radialblend':

            from poison_tool_box import radialblend
            poison_transform = radialblend.poison_transform(img_size=img_size, denormalizer=denormalizer, alpha=alpha, normalizer=normalizer,
                                                      target_class=target_class)
