...
if args.poison_type == 'dynamic':
    ...
    if args.dataset == 'cifar10':
        ...
        ckpt_path = './models/all2one_cifar10_ckpt.pth'
        ...
        ckpt_path = './models/all2one_gtsrb_ckpt.pth'
    ...
    if args.dataset == 'gtsrb':
        ...
        train_set = datasets.ImageFolder(os.path.join(os.path.join(data_dir, 'imagenet200'), 'train'),
                                         data_transform)
        img_size = 224
        num_classes = 200
...
if args.poison_type in ['basic', 'badnet', 'blend', 'clean_label', 'refool',
                        ...
                        'SIG', 'TaCT', 'WaNet', 'Mirror', 'SleeperAgent', 'none',
                        'badnet_all_to_all', 'trojan', 'radialblend']:
    ...
    if args.poison_type == 'basic':
        ...
        poison_generator = badnet.poison_generator(img_size=img_size, dataset=train_set,
                                                   poison_rate=args.poison_rate, trigger_mark=trigger,
    ...
    elif args.poison_type == 'Mirror':
        kappa = 0.1
        cx = 0.5
        cy = 0.5

        from poison_tool_box import Mirror

        poison_generator = Mirror.poison_generator(img_size=img_size, dataset=train_set,
                                                  poison_rate=args.poison_rate, cover_rate=args.cover_rate,
                                                  path=poison_set_dir, kappa=kappa, cx=cx, cy=cy,
                                                  target_class=config.target_class[args.dataset])

    elif args.poison_type == 'radialblend':

        from poison_tool_box import radialblend

        poison_generator = radialblend.poison_generator(img_size=img_size, dataset=train_set,
                                                  poison_rate=args.poison_rate, cover_rate=args.cover_rate,
                                                  path=poison_set_dir,
                                                  target_class=config.target_class[args.dataset], alpha=alpha)
        ...
        poison_generator = adaptive_patch.poison_generator(img_size=img_size, dataset=train_set,
                                                           poison_rate=args.poison_rate,
                                                           path=poison_set_dir,
                                                           trigger_names=config.adaptive_patch_train_trigger_names[
                                                               args.dataset],
                                                           alphas=config.adaptive_patch_train_trigger_alphas[
                                                               args.dataset],
                                                           target_class=config.target_class[args.dataset],
                                                           cover_rate=args.cover_rate)
    ...
    if args.poison_type not in ['TaCT', 'WaNet', 'Mirror', 'adaptive_blend', 'adaptive_patch', 'adaptive_k_way', 'radialblend']:
    ...
    poison_generator = ISSBA.poison_generator(ckpt_path=ckpt_path, secret=secret, dataset=train_set,
